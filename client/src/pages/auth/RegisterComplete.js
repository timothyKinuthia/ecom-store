...
const RegisterComplete = ({history}) => {
    ...
    const handleSubmit = async evt => {
        ...
        try {
            ...
            if (result.user.emailVerified) {
                ...
                createUdateUser(idTokenResult.token).then(res => {
                    dispatch({
   ...
   const completeRegistrationForm = () => <form onSubmit={handleSubmit}>
       ...
       <input type="password" value={password} className="form-control" onChange={evt => setPassword(evt.target.value) } placeholder="password" autoFocus/>
